...
`));const e={eval:{type:String,alias:"e"},print:{type:String,alias:"p"}},{_:u,flags:s}=Vu({flags:{...e,inputType:String,test:Boolean},help:!1,ignoreArgv:at(!1)}),n=yn({...bn,...e}),i=["print","eval"].find(l=>!!s[l]);if(i){const{inputType:l}=s,f=s[i],{transformSync:h}=await import("esbuild"),c=h(f,{loader:"default",sourcefile:"/eval.ts",format:l==="module"?"esm":"cjs"});n.unshift(`--${i}`,c.code)}yu(In)&&s.test&&u.length===0&&n.push("**/{test,test/**/*,test-*,*[.-_]test}.?(c|m)@(t|j)s");const o=await wn(),D=as(n,{noCache:!!t.flags.noCache,tsconfigPath:t.flags.tsconfig});yl(D,o),process.send&&D.on("message",l=>{process.send(l)}),D.send&&process.on("message",l=>{D.send(l)}),D.on("close",l=>{l===null&&(l=ft.signals[D.signalCode]+128),process.exit(l)})});
